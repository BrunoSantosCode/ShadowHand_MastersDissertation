...
import csv
import sys
sys.path.append("/home/user/projects/shadow_robot/base/src/teach_net_teleoperation")
...
args_cuda = rospy.get_param('~cuda', False)
args_gpu = rospy.get_param('~gpu', 0)
args_model_path = rospy.get_param('~model_path', '/home/user/projects/shadow_robot/base/src/teach_net_teleoperation/weights/new_early_teach_teleop.model')

args_cuda = args_cuda if torch.cuda.is_available else False
...
if args_cuda:
    model = torch.load(args_model_path, map_location='cuda')
else:
    model = torch.load(args_model_path, map_location='cpu')
model.device_ids = [args_gpu]
print('load model {}'.format(args_model_path))
...
class Teleoperation():
    def __init__(self):
        ...
        rospy.Subscriber("/kinect2/sd/image_depth", Image, self.callback)
    ...
    def callback(self, img_data):
        ...
        #middle_pixel_value = self.img[self.img.shape[0]//2, self.img.shape[1]//2]
        #print("Middle pixel value:", middle_pixel_value)
