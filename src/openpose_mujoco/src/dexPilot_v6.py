...
E1 = 0.03       # Threshold for pinching position principal vectors (m)
E2 = 0.03       # Threshold for pinching position secundary vectors (m)
E3 = 0.05       # Threshold for closed fingers (m)
BETA = 1.0      # Scale factor (Shadow Hand <-> Human Hand)
ETA1 = 0.0001   # Closes distance finger/thumb (m)
ETA2 = 0.03     # Forces minimum separation between primary fingers (m)
ETA3 = 0.01     # Forces closed finger positions (m)
GAMMA = 0.003   # Weight on regularizing the robotic hand angles to zero (fully opened hand)
...
def getThumbTip(joints):
    ...
    z = x0*cos(PI/4) - z0*sin(PI/4)
    x = x0*sin(PI/4) + z0*cos(PI/4)
    y = -y0
    z = z + 2.9
    x = x - 3.4
    y = y + 0.85
...
def getLittleFingerTip(joints):
    ...
    z = x0*cos(radians(55)) + z0*sin(radians(55))
    x = x0*sin(radians(55)) - z0*cos(radians(55))
    y = y0
    z = z + 2.071
    x = x + 1.1 + 2.2
    return np.array([x*0.01, y*0.01, z*0.01])   # [cm -> m]
...
bounds_deg = ((0,0), (0,90), (-15,90), (-20,20),
              (0,0), (0,90), (-15,90), (-20,20), (0,45),
              (0,0), (0,90), (-15,90), (-20,20),
              (0,0), (0,90), (-15,90), (-20,20),
...
def dexPilot_joints(hand_kp):
    ...
    aux0 = np.array([hand_kp[0*3+0], hand_kp[0*3+1], hand_kp[0*3+2]])
    aux5 = np.array([hand_kp[5*3+0], hand_kp[5*3+1], hand_kp[5*3+2]])
    aux9 = np.array([hand_kp[9*3+0], hand_kp[9*3+1], hand_kp[9*3+2]])
    aux13 = np.array([hand_kp[13*3+0], hand_kp[13*3+1], hand_kp[13*3+2]])
    wr = np.array([hand_kp[0*3+0], hand_kp[0*3+1], hand_kp[0*3+2]])
    th = np.array([hand_kp[4*3+0], hand_kp[4*3+1], hand_kp[4*3+2]])
    ff = np.array([hand_kp[8*3+0], hand_kp[8*3+1], hand_kp[8*3+2]])
